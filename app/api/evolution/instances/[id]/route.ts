...
export async function PUT(
  ...
  try {
    ...
    // Verificar ação
    if (!['connect', 'restart', 'logout', 'set_default', 'unset_default'].includes(action)) {
      return NextResponse.json(
        { error: 'Ação inválida' },
        { status: 400 }
      )
    ...
    if (action === 'set_default') {
      // Primeiro remove o default de todas as instâncias
      await executeQuery(
        'UPDATE evolution_instancias SET is_default = FALSE'
      )
      
      // Define esta instância como padrão
      await executeQuery(
        'UPDATE evolution_instancias SET is_default = TRUE WHERE id = ?',
        [instance.id]
      )

      return NextResponse.json({
        message: 'Instância definida como padrão para notificações'
      })
    }
    if (action === 'unset_default') {
      await executeQuery(
        'UPDATE evolution_instancias SET is_default = FALSE WHERE id = ?',
        [instance.id]
      )
      return NextResponse.json({
        message: 'Instância removida como padrão'
      })
    }
